...
mobile_wumpus = False
cave = [[0 for x in range(GRID_WIDTH)] for y in range(GRID_HEIGHT)]
...
def draw_room( pos, screen):
    ...
    if (pos[x]>0 and pos[x]<GRID_WIDTH-1) and (pos[y]>0 and pos[y]<GRID_HEIGHT-1):
        ...
        filename = './images/fourexits'+str( cave[pos[x]][pos[y]])+'.PNG'
        #print (filename)
        four_exits = pygame.image.load(filename)
        screen.blit( four_exits, (0,0) )
    ...
    if wumpus_near == True:
        wumpus_text = font.render("A horrible stench assaults your nostrils.", 1, (0, 255, 64))
        y_text_pos = y_text_pos+wumpus_text.get_height()+10
        screen.blit(wumpus_text,(0, y_text_pos))
    if bats_near == True:
...
def populate_cave():
    ...
    center_rooms = [x for x in range(1,26)]
    random.shuffle(center_rooms)
    #place the player
    ...
    for x in range(1, 6):
        for y in range(1, 6):
            tile_num = center_rooms.pop()
            if [x,y] in pits_list:
                cave[x][y]= 26
                print ("Pit tile at:"+str([x,y]))
                continue
            elif tile_num > 14:
                cave[x][y] = 0
            else:
...
#four_exits = pygame.image.load('images/fourexits0.png')
